...
extension Path {
    ...
    struct Components {
        ...
        func includeCurrentDirectory() -> Bool {
            ...
            case (.some(ASCII.fullStop.rawValue), .none):
                return true
            case let (.some(ASCII.fullStop.rawValue), .some(second)):
...
extension Path.Components {
    ...
    func parseSingleComponent(_ bytes: Bytes) -> Path.Component? {
        ...
        switch bytes {
        case ASCII.fullStop.rawValue where prefixVerbatim:
            return Path.Component.current()
        case ASCII.fullStop.rawValue:
            return nil
        case [ASCII.fullStop.rawValue, ASCII.fullStop.rawValue]:
            return Path.Component.parent()
        default:
            return Path.Component(.regular(OsString(unchecked: bytes)))
        }
    }
